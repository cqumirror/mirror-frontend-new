 ...
 *     /github-release/OWNER/REPO/... → "github-release"
 */
...
function mergeIsoInfo(
  ...
  for (const entry of isoData) {
    // 从第一个 url 路径推断 mirror id（如 /ubuntu-releases/... → "ubuntu-releases"）
    const mirrorId = entry.urls.length > 0 ? inferMirrorIdFromUrl(entry.urls[0].url) : null;
